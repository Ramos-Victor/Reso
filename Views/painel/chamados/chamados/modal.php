<?php
    function ListarEquipamentos($categoria = null, $sala = null) {
        $sql = 'SELECT 
                ...
                WHERE e.st_equipamento = 1 AND e.st_ativo = 1 AND  e.id_unidade = ? AND s.nm_sala !="ESTOQUE"';
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <form method="post">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btn-success" name="action" value="Avaliar">Confirmar Avaliação</button>
                    </div>
                </form>
            </div>
        </div>
